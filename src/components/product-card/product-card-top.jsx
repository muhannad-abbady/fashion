    ...
    return (
        <Box position="relative" mb={1}
            ...
            <Box className="tags" position="absolute" top={0} left={0} sx={{ zoom: { xs: 0.8, sm: 1 } }}>
                {(Math.random() < 0.1) ? <Tag text="New" bgcolor={cssVars.primary} /> : null}
                {(Math.random() < 0.1) ? <Tag text="Sale" bgcolor={cssVars.red} /> : null}
            </Box>
            <Box className="actions" position="absolute" top={0} sx={{ transition: "all 0.25s ease-in", right: { xs: 0, md: -36 }, zoom: { xs: 0.8, sm: 1 } }}>
                ...
                <ActionIconButton icon={<CompareArrowsIcon />} iconFilled={<CompareArrowsIcon />} active={isCompared()} onClick={(e) => { }} isLoading={false} />
            </Box>
        </Box>
    );
}
